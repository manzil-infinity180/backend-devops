...
func CreateComment(db *pg.DB, req *Comment) (*Comment, error) {
	...
	comment := &Comment{}
	err = db.Model(comment).Relation("User").Where("comment.id = ?", req.ID).Select()
	return comment, err
}
...
func GetComments(db *pg.DB) ([]*Comment, error) {
	...
	err := db.Model(&comments).Relation("User").Select()
	return comments, err
}
...
func DeleteComment(db *pg.DB, commentID int64) error {
	comment := &Comment{}
	err := db.Model(comment).Relation("User").Where("comment.id = ?", commentID).Select()
	if err != nil {
		return err
	}
